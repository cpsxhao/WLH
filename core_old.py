...
login_data = {'userid':'', 'userpass':''}       #id与password自行输入
global course_info      #[{'url':'**', 'name':'**', 'undone_homework':'**', 'new_notice':'**', 'new_file':'**'}, ...]
course_info = []        #新版网络学堂url地址长度为76，旧版url为64
global course           #[{name:'**', notice:[{'rank':'**', 'title':'**', 'release':'**', 'time':'**', 'ifread':'**', 'content':'**'},...],
...
def logIn(userid, userpass):     #抓取所有文本信息
    ...
    for i in range(len(course_url)):
        if len(course_url[i]) < 70:   #限定旧版网络学堂
    ...
    for i in range(len(old_course_url)):
        course[i]['notice'] = []            #notice:[{'rank':'**', 'title':'**', 'release':'**', 'time':'**', 'ifread':'**', 'content':'**'},...]
        ...
        for n in notice:
            ...
            for j in range(tem.count('')):
                tem.remove('')
            r = s.get('http://learn.tsinghua.edu.cn/MultiLanguage/public/bbs/' + n.a.get('href'))
            ...
            course[i]['notice'].append({'rank': tem[0], 'title': tem[1], 'release': tem[2], 'time': tem[3], 'ifread': tem[4], 'content': content})
...
#    get_homework_file(url=course[6]['homework'][0]['title_url'], path='F:\\')
